...
# Create the parser
parser = argparse.ArgumentParser(description="parameters: 'max_noise_space'and 'noise_type'")
...
for s in s_values:
    ...
    accuracy, accuracy_direct = run_exp(num_training_samples = 20000, num_labled_samples=s,noise_level=args.max_noise_space, noise_type = args.noise_type)
...
results.to_csv('mnist_results/mnist.csv', index=False)
...
plt.savefig('mnist_results/mnist_noise'+str(args.noise_type) +'.png', dpi=300, bbox_inches='tight')
